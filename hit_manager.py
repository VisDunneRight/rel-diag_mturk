...
def get_worker_id_list(hit_id):
    ...
    assignments = conn.list_assignments_for_hit(
        HITId=hit_id, MaxResults=100, AssignmentStatuses=["Rejected"]
    )["Assignments"]
    for i in assignments:
        print(i["WorkerId"])
        worker_id_list.append(i["WorkerId"])
...
def approve_qualifications(qual_id):
    ...
    for worker in list_workers_with_qual:
        print(
            "worker_id "
            + worker["WorkerId"]
            + " was granted the qualification on "
...
def set_taken_test_qualification(qual_id, workers_file):
    ...
    for i in range(len(worker_ids)):
        ...
        print(
            "Worker ID: "
            + worker_ids[i]
            + " has been given the taken_test_qualification"
        )
...
def remove_qualification(qual_id, workers_file):
    ...
    for i in range(len(worker_ids)):
        ...
        print(
            "Worker ID: "
            + worker_ids[i]
            + " has been revoked from the taken_test_qualification"
        )
...
def get_workers_with_qualification(qual_id):
    ...
    def print_qual_results(response, success_count, reject_count):
        ...
        for qual in response["Qualifications"]:
            ...
            print(
                ...
                + str(qual["GrantTime"])
                + " with IntegerValue of "
                + str(qual["IntegerValue"])
            )
    ...
    print(
        "There are in total "
        + str(success_count + reject_count)
        + " workers that have attempted the qualification "
        + qual_id
    )
...
def get_qualification_score(qual_id, worker_id):
    response = conn.get_qualification_score(
        QualificationTypeId=qual_id, WorkerId=worker_id
    )["Qualification"]
    ...
    print(
        ...
        + response["Status"]
        + " the qualification on "
        + str(response["GrantTime"])
        + " with IntegerValue of "
        + str(response["IntegerValue"])
    )
...
def update_expiration(hit_id):
    response = conn.update_expiration_for_hit(
        HITId=hit_id, ExpireAt=datetime(2019, 5, 15, 18, 0, 0)
    )
    print(response)
...
def notify_workers(workers_file):
    ...
    for worker in worker_ids:
        ...
        response = conn.notify_workers(
            ...
            MessageText=(
                "You have previously participated in our HIT regarding the interpretation of SQL queries through "
...
def notify_workers_with_qualification(qualification_file, taken_hit_file):
    ...
    for worker in workers_to_notify:
        ...
        response = conn.notify_workers(
            Subject="Invitation to participate in: Understanding SQL Queries -- $5.20 to $16.47 with bonuses HIT",
            MessageText=(
                "We noticed that you have successfully completed your qualification test for our 'Understanding SQL Queries' HIT,"
...
if __name__ == "__main__":
    ...
    if arg1 == "summary":
        ...
        arg1 == "approve_qualifications"
    ):  # !!!WARNING!!! hard-coded `accept_list` in `approve_qualifications` definition.
        qual_id = arg_arr[1]
        approve_qualifications(qual_id)
    elif (
        arg1 == "update_expiration"
    ):  # !!!WARNING!!! hard-coded `ExpireAt` in `update_expiration` definition.
        hit_id = arg_arr[1]
        update_expiration()
        ...
        arg1 == "notify_workers_with_qualification"
    ):  # **!!!WARNING!!!** Hard-coded advertisement message.
